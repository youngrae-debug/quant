    ...
    <main className="mx-auto max-w-6xl px-6 py-12">
      ...
      <section className="mt-6 rounded-xl border border-slate-800 bg-slate-900/40 p-6">
        ...
        {dcfScenarios.length === 0 ? (
          <p className="mt-4 text-sm text-slate-400">Insufficient price data for DCF range.</p>
        ) : (
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {dcfScenarios.map((scenario) => (
                  <tr key={`${scenario.growth}-${scenario.discount}`} className="border-t border-slate-800">
                    ...
                    <td className={`px-4 py-3 text-right ${scenario.upside != null && scenario.upside >= 0 ? 'text-emerald-300' : 'text-rose-300'}`}>
